...
interface ExistingUpdateDay {
  date: string;
  public?: { message: { en: string; fa: string } }[];
}
...
async function generateUpdates() {
  try {
    ...
    // Increased maxBuffer to 10MB to avoid overflow
    const { stdout } = await execAsync(
      'git log --pretty=format:"%H%x00%ad%x00%s%x00%an" --date=short -n 500',
    ...
    if (fs.existsSync(outputPath)) {
      try {
        const existingData: ExistingUpdateDay[] = JSON.parse(fs.readFileSync(outputPath, 'utf-8'));
        if (Array.isArray(existingData)) {
          for (const day of existingData) {
            if (day.public) {
              for (const update of day.public) {
                if (
                  update.message &&
    ...
    const processed: UpdateDay[] = Object.keys(grouped)
      ...
      .map(date => {
        ...
        const uniquePublic = Array.from(
          new Map(publicUpdates.map(item => [item.message, item])).values()
        );
        ...
        return {
          ...
          public: uniquePublic.map(item => {
            ...
            const cachedFa = translationCache[item.message as string];
            return {
              ...item,
              message: { en: item.message as string, fa: cachedFa || (item.message as string) }, // Use cached or fallback to English
            };
          }),
        };
      })
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      for (const day of processed) {
        for (const update of day.public) {
          ...
          const msg = update.message as { en: string; fa: string };
          if (msg.en === msg.fa && !translationCache[msg.en]) {
            textsToTranslate.push({ date: day.date, hash: update.hash, text: msg.en });
          }
        }
      }
      ...
      if (textsToTranslate.length > 0) {
              ...
              messages: [
                {
                  role: 'system',
                  content:
                    'You are a helpful translator for a software project changelog. Translate to formal but modern Persian.',
                },
        ...
        for (const day of processed) {
          for (const update of day.public) {
            const msg = update.message as { en: string; fa: string };
            const translated = translationCache[msg.en];
            if (translated) {
              msg.fa = translated;
            }
          }
    ...
    console.warn(
      'Warning: Could not generate updates from git log. Using empty updates list.',
      error
    );
