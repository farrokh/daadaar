...
export default function ReportsPage() {
  ...
  const fetchReports = async (pageNum: number, searchQuery: string = '') => {
    ...
    setError(null);
    try {
      const response = await fetchApi<{ reports: ReportWithDetails[], pagination: { totalPages: number } }>(
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 w-full md:w-auto">
            ...
            <Button type="submit" className="bg-accent-primary hover:bg-accent-primary/80 text-white font-bold rounded-2xl">
              {commonT('search')}
            </Button>
          </form>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
           <span className="text-xs text-foreground/40 uppercase tracking-widest font-bold mb-2 w-full">{commonT('filter')}</span>
           <Button variant="outline" size="sm" className="bg-foreground/5 border-foreground/10 text-foreground/60 hover:text-foreground hover:bg-foreground/10 rounded-full text-xs">
        ...
        {loading && reports.length === 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-foreground/5 border border-foreground/10 rounded-2xl h-96 animate-pulse" />
            ))}
          </div>
        )}
      </div>
