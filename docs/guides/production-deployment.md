...
- Command: `aws ec2 describe-vpc-endpoints --query "VpcEndpoints[*].{ID:VpcEndpointId, Service:ServiceName, SubnetCount:length(SubnetIds)}"`
- **Expectation**: `SubnetCount` count should be **1** (or 2). If it is high (e.g. 6), immediate action is required to remove subnets.
- **Accuracy Note**: For a strictly accurate unique-AZ count, list each endpoint's `SubnetIds` and then call `describe-subnets` to map those IDs to `AvailabilityZone` identifiers, as multiple subnets can theoretically exist in the same AZ.
