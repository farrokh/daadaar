...
2. **Redis (Upstash)**: Ephemeral storage for sessions, rate limits, and PoW challenges. **Required runtime dependency** for production. See [Infrastructure docs](infrastructure.md#-redis-configuration--operations) for configuration and deployment details.
